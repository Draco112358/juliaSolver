...
function gmres_custom(b, restarted, tol, maxit, x , wk, incidence_selection, FFTCP, FFTCLp, DZ, Yle, expansions, invZ, invP, lu, PLIVector, PVector, PLI2Vector, P2Vector, time1Vector, time2Vector)
    # @profile begin
    ...
    r = b - ComputeMatrixVector(x , wk, incidence_selection, FFTCP, FFTCLp, DZ, Yle, expansions, invZ, invP, lu, PLIVector, PVector, PLI2Vector, P2Vector, time1Vector, time2Vector);
    normr = norm(r)
    if normr <= tolb
    ...
    for outiter = 1:outer 
        ...
        for initer = 1:inner
            ...
            # Apply A to v.
            v = ComputeMatrixVector(v , wk, incidence_selection, FFTCP, FFTCLp, DZ, Yle, expansions, invZ, invP, lu, PLIVector, PVector, PLI2Vector, P2Vector, time1Vector, time2Vector);
            #println(norm(v))
            # Form Pj*Pj-1*...P1*Av.
            for k = 1:initer
                Utemp = @view U[:, k]
            ...
            if (normr <= tolb || stag >= maxstagsteps || moresteps == 1)
                ...
                r = b - ComputeMatrixVector(xm , wk, incidence_selection, FFTCP, FFTCLp, DZ, Yle, expansions, invZ, invP, lu, PLIVector, PVector, PLI2Vector, P2Vector, time1Vector, time2Vector);
                if norm(r) <= tol * n2b
                    x = xm
                    flag = 0
                    iter = [outiter, initer]
